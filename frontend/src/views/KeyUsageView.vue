    ...
    <header class="relative z-20 px-6 py-4">
      <nav class="mx-auto flex max-w-6xl items-center justify-between">
        <router-link to="/home" class="flex min-w-0 items-center">
          <BrandLogo
            variant="compact"
            class="h-10 w-auto max-w-[230px] object-contain sm:max-w-[280px]"
          />
        </router-link>
    ...
    <main class="flex-1 w-full max-w-5xl mx-auto px-6 py-12">
      ...
      <div v-if="showResults">
        ...
        <div v-else-if="resultData" class="space-y-6">
          ...
          >
            <div class="flex flex-col gap-3 px-8 py-5 border-b border-accent-200 dark:border-dark-700 sm:flex-row sm:items-center sm:justify-between">
              <h3 class="text-sm font-semibold uppercase tracking-wider text-accent-500 dark:text-dark-400">{{ t('keyUsage.dailyDetail') }}</h3>
              <div class="inline-flex rounded-lg border border-accent-200 bg-white p-0.5 dark:border-dark-700 dark:bg-dark-950">
                <button
                  ...
                  :class="dailyUsageDays === option.value
                    ...
                    : 'text-accent-600 hover:bg-accent-100 dark:text-dark-300 dark:hover:bg-dark-800'"
                >
                  {{ option.label }}
                </button>
              </div>
              ...
              <table class="w-full">
                <thead>
                  <tr class="border-b border-accent-200 bg-accent-50 dark:border-dark-700 dark:bg-dark-950">
                    <th class="px-4 py-3 text-left text-xs font-semibold uppercase tracking-wider text-accent-500 dark:text-dark-400">{{ t('keyUsage.date') }}</th>
                    ...
                    <th class="px-4 py-3 text-right text-xs font-semibold uppercase tracking-wider text-accent-500 dark:text-dark-400">{{ t('keyUsage.cacheWriteTokens') }}</th>
                ...
                <tbody>
                  ...
                  >
                    <td class="px-4 py-3 text-sm font-medium whitespace-nowrap text-accent-900 dark:text-white">{{ row.date }}</td>
                    <td class="px-4 py-3 text-sm tabular-nums text-right text-accent-700 dark:text-dark-200">{{ fmtNum(row.requests) }}</td>
                    <td class="px-4 py-3 text-sm tabular-nums text-right text-accent-700 dark:text-dark-200">{{ fmtNum(row.input_tokens) }}</td>
                    <td class="px-4 py-3 text-sm tabular-nums text-right text-accent-700 dark:text-dark-200">{{ fmtNum(row.output_tokens) }}</td>
                    <td class="px-4 py-3 text-sm tabular-nums text-right text-accent-700 dark:text-dark-200">{{ fmtNum(row.cache_read_tokens) }}</td>
                    <td class="px-4 py-3 text-sm tabular-nums text-right text-accent-700 dark:text-dark-200">{{ fmtNum(row.cache_write_tokens) }}</td>
                    <td class="px-4 py-3 text-sm tabular-nums text-right font-medium text-accent-900 dark:text-white">{{ usd(row.actual_cost != null ? row.actual_cost : row.cost) }}</td>
                  </tr>
                </tbody>
              </table>
            </div>
            <div v-else class="px-8 py-8 text-center text-sm text-accent-500 dark:text-dark-400">
...
const RING_GRADIENTS = [
  { from: '#96611d', to: '#d7a84f' },
  { from: '#b88733', to: '#c5a989' },
...
.input-ring:focus {
  box-shadow: 0 0 0 3px rgba(150, 97, 29, 0.2);
  border-color: #96611d;
  outline: none;
}
...
.skeleton {
  background: linear-gradient(90deg, #dfcfb9 25%, #faf7f2 50%, #dfcfb9 75%);
  background-size: 200% 100%;
  animation: shimmer-kv 1.8s ease-in-out infinite;
  border-radius: 8px;
}
:global(.dark) .skeleton {
  background: linear-gradient(90deg, #3b2b22 25%, #251c18 50%, #3b2b22 75%);
  background-size: 200% 100%;
}
