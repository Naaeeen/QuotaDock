<template>
  <div class="min-h-screen bg-accent-50 text-accent-900 dark:bg-dark-950 dark:text-white">
    <header class="border-b border-accent-200 bg-white/95 dark:border-dark-800 dark:bg-dark-900/95">
      <div class="mx-auto flex max-w-5xl items-center justify-between gap-4 px-4 py-4 sm:px-6">
        <RouterLink to="/home" class="flex min-w-0 items-center">
          <BrandLogo
            variant="compact"
            :settings="settings"
            class="h-9 w-auto max-w-[220px] object-contain sm:max-w-[260px]"
          />
        </RouterLink>
        <RouterLink
          to="/login"
    ...
    <main class="mx-auto max-w-4xl px-4 py-8 sm:px-6 lg:py-10">
      ...
      <section
        ...
        class="rounded-lg border border-accent-200 bg-white p-6 dark:border-dark-700 dark:bg-dark-900"
      >
        <div class="flex items-start gap-3">
          <span class="flex h-10 w-10 flex-shrink-0 items-center justify-center rounded-md bg-accent-100 text-accent-600 dark:bg-dark-800 dark:text-dark-300">
            <Icon name="document" size="sm" />
          </span>
          <div>
            <h1 class="text-lg font-semibold text-accent-900 dark:text-white">{{ t('legal.notFound') }}</h1>
      ...
      <article v-else>
        <div class="mb-8 border-b border-accent-200 pb-6 dark:border-dark-700">
          <div class="flex items-start gap-4">
            ...
            <div class="min-w-0">
              ...
              <h1 class="mt-2 break-words text-2xl font-bold tracking-normal text-accent-950 dark:text-white sm:text-3xl">
                {{ currentDocument.title }}
              </h1>
              <p v-if="updatedAt" class="mt-3 text-sm text-accent-500 dark:text-dark-400">
        ...
        <div
          v-else
          class="rounded-lg border border-dashed border-accent-300 bg-white px-6 py-14 text-center text-sm text-accent-500 dark:border-dark-700 dark:bg-dark-900 dark:text-dark-400"
        >
          {{ t('legal.empty') }}
        </div>
      </article>
...
.legal-document-content :deep(h1) {
  @apply mb-4 mt-8 border-b border-accent-200 pb-3 text-3xl font-bold dark:border-dark-700;
}
...
.legal-document-content :deep(p) {
  @apply mb-4 text-accent-700 dark:text-dark-200;
}
...
.legal-document-content :deep(li) {
  @apply mb-1 text-accent-700 dark:text-dark-200;
}

.legal-document-content :deep(blockquote) {
  @apply my-5 border-l-4 border-accent-300 pl-4 text-accent-600 dark:border-dark-600 dark:text-dark-300;
}

.legal-document-content :deep(code) {
  @apply rounded bg-accent-100 px-1.5 py-0.5 font-mono text-sm dark:bg-dark-800;
}

.legal-document-content :deep(pre) {
  @apply my-5 overflow-x-auto rounded-lg bg-accent-950 p-4 text-accent-100;
}
...
.legal-document-content :deep(th) {
  @apply border border-accent-300 bg-accent-50 px-3 py-2 text-left font-semibold dark:border-dark-600 dark:bg-dark-800;
}

.legal-document-content :deep(td) {
  @apply border border-accent-300 px-3 py-2 dark:border-dark-600;
}
...
.legal-document-content :deep(hr) {
  @apply my-7 border-accent-200 dark:border-dark-700;
}
</style>
