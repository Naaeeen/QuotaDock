  ...
  <AppLayout>
      ...
      <div v-if="showGenerateDialog" class="fixed inset-0 z-50 flex items-center justify-center">
        ...
        >
          ...
          <form @submit.prevent="handleGenerateCodes" class="space-y-4">
            ...
            <!-- 邀请码类型：显示提示信息 -->
            <div v-if="generateForm.type === 'invitation'" class="rounded-lg bg-primary-50 p-3 dark:bg-primary-900/20">
                ...
                <button
                  ...
                  :class="[
                    ...
                    generateForm.expiry_option === option.value
                      ...
                      : 'border-accent-200 text-accent-700 hover:bg-accent-50 dark:border-dark-600 dark:text-accent-300 dark:hover:bg-dark-700'
                  ]"
                >
                  {{ option.label }}
                </button>
    ...
    <Teleport to="body">
      ...
      >
        ...
        >
          <h2 class="mb-1 text-lg font-semibold text-accent-900 dark:text-white">
            {{ t('admin.redeem.batchUpdateTitle') }}
          </h2>
          <p class="mb-4 text-sm text-accent-500 dark:text-accent-400">
            {{ t('admin.redeem.selectedCount', { count: selectedCount }) }}
          </p>

          <form data-test="batch-update-form" class="space-y-4" @submit.prevent="handleBatchUpdate">
            <div class="space-y-2">
              <label class="flex items-center gap-2 text-sm font-medium text-accent-700 dark:text-accent-300">
                <input
