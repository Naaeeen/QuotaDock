            ...
            <template #after>
              ...
              <div class="relative" ref="accountToolsDropdownRef">
                ...
                <div
                  ...
                  class="absolute right-0 z-50 mt-2 w-[min(20rem,calc(100vw-2rem))] origin-top-right overflow-hidden rounded-lg border border-accent-200 bg-white shadow-xl dark:border-accent-700 dark:bg-accent-800"
                >
                  <div class="max-h-[70vh] overflow-y-auto p-2">
                    <div class="px-2 py-2">
                      <div class="text-xs font-semibold uppercase tracking-wide text-accent-400 dark:text-accent-500">
                    ...
                    <div class="my-2 border-t border-accent-100 dark:border-accent-700"></div>
                    <div class="px-2 py-2">
                      <div class="text-xs font-semibold uppercase tracking-wide text-accent-400 dark:text-accent-500">
                    ...
                    <button class="account-tools-menu-item" @click="openTLSFingerprintProfiles">
                      <span class="account-tools-menu-icon bg-accent-100 text-accent-600 dark:bg-accent-700 dark:text-accent-200">
                    ...
                    <div class="px-2 py-2">
                      <div class="flex items-center justify-between gap-3">
                        <span class="text-xs font-semibold uppercase tracking-wide text-accent-400 dark:text-accent-500">
                          {{ t('admin.accounts.viewColumns') }}
                        </span>
                        <Icon name="grid" size="sm" class="text-accent-400" />
                      </div>
                    </div>
                    <div class="grid grid-cols-1 gap-1">
                      <button
                        ...
                        class="flex w-full items-center justify-between rounded-md px-3 py-2 text-sm text-accent-700 transition-colors hover:bg-accent-100 dark:text-accent-200 dark:hover:bg-accent-700"
                      >
      ...
      <template #table>
        ...
        >
          ...
          <template #cell-id="{ value }">
            <span class="font-mono text-xs text-accent-500 dark:text-accent-400">#{{ value }}</span>
          </template>
          ...
          <template #cell-proxy="{ row }">
            <div class="flex flex-col gap-1">
              ...
              <div v-if="row.proxy && row.proxy.expires_at" class="flex items-center gap-2 text-xs">
                <span class="text-accent-600 dark:text-accent-300">{{ formatDateTime(row.proxy.expires_at) }}</span>
              ...
              <div v-if="row.proxy_fallback_origin_id" class="flex items-center gap-1">
                ...
                <button class="text-xs px-1.5 py-0.5 rounded border border-accent-300 dark:border-accent-600 text-accent-600 dark:text-accent-300 hover:bg-accent-100 dark:hover:bg-accent-700" @click="onRevertFallback(row)">{{ t('admin.accounts.revertProxy') }}</button>
              </div>
            </div>
          </template>
  ...
  switch (state) {
    ...
    case 'auto':
      return {
        ...
        dotClass: 'bg-accent-300 dark:bg-accent-500'
      }
  }
}
...
function getAntigravityTierClass(row: any): string {
  ...
  switch (tier) {
    ...
    case 'g1-pro-tier': return 'bg-primary-100 text-primary-600 dark:bg-primary-900/40 dark:text-primary-300'
    case 'g1-ultra-tier': return 'bg-accent-100 text-accent-600 dark:bg-accent-900/40 dark:text-accent-300'
    default: return ''
  }
}
...
const handleExportData = async () => {
  ...
  try {
    ...
    const filename = `quotadock-account-${timestamp}.json`
...
.account-tools-menu-item {
  @apply flex w-full items-center gap-3 rounded-md px-3 py-2 text-sm text-accent-700 transition-colors hover:bg-accent-100 dark:text-accent-200 dark:hover:bg-accent-700;
}
