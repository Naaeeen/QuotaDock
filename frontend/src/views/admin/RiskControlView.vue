    ...
    <div class="space-y-6">
      ...
      <template v-else>
        <div class="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
          <div>
            <h1 class="text-2xl font-semibold text-accent-900 dark:text-white">{{ t('admin.riskControl.title') }}</h1>
            <p class="mt-1 text-sm text-accent-500 dark:text-accent-400">{{ t('admin.riskControl.description') }}</p>
          </div>
        ...
        <div class="grid grid-cols-1 gap-3 md:grid-cols-2 xl:grid-cols-4">
          <div
            ...
            :key="item.key"
            class="rounded-lg border border-accent-100 bg-white px-4 py-3 shadow-sm dark:border-dark-700 dark:bg-dark-800"
          >
            <div class="flex min-w-0 items-center gap-3">
              ...
              <div class="min-w-0 flex-1">
                <div class="flex min-w-0 items-center justify-between gap-2">
                  <p class="truncate text-xs font-medium text-accent-500 dark:text-accent-400">{{ item.label }}</p>
                  <span
                    v-if="item.badge"
                ...
                <div class="mt-1 flex min-w-0 items-baseline gap-2">
                  <p class="truncate text-xl font-semibold leading-7 text-accent-900 dark:text-white">{{ item.value }}</p>
                  <p v-if="item.meta" class="truncate text-xs text-accent-500 dark:text-accent-400">{{ item.meta }}</p>
                </div>
              </div>
            </div>
          </div>
        ...
        >
          <div data-test="pre-block-sync-card" class="card">
            <div class="flex flex-col gap-4 border-b border-accent-100 px-6 py-4 dark:border-dark-700 lg:flex-row lg:items-center lg:justify-between">
              <div>
                <h2 class="text-lg font-semibold text-accent-900 dark:text-white">{{ t('admin.riskControl.preBlockSyncStatus') }}</h2>
                <p class="mt-1 text-sm text-accent-500 dark:text-accent-400">{{ t('admin.riskControl.preBlockSyncHint') }}</p>
              </div>
              <span class="inline-flex w-fit items-center rounded-full bg-accent-100 px-2.5 py-1 text-xs font-medium text-accent-600 dark:bg-dark-700 dark:text-accent-300">
            ...
            <div class="p-6">
              <div data-test="pre-block-metric-grid" class="grid grid-cols-2 gap-3 md:grid-cols-3">
                <div
                  ...
                  :class="item.class"
                >
                  <p class="text-xs text-accent-500 dark:text-accent-400">{{ item.label }}</p>
                  <p class="mt-2 truncate text-2xl font-semibold leading-8" :class="item.valueClass">{{ item.value }}</p>
                  <p v-if="item.meta" class="mt-1 truncate text-xs text-accent-500 dark:text-accent-400">{{ item.meta }}</p>
                </div>
              </div>
            </div>
          </div>
          ...
          <div data-test="pre-block-api-key-load-card" class="card">
            <div class="flex flex-col gap-4 border-b border-accent-100 px-6 py-4 dark:border-dark-700 lg:flex-row lg:items-center lg:justify-between">
              <div>
                <h2 class="text-lg font-semibold text-accent-900 dark:text-white">{{ t('admin.riskControl.preBlockAPIKeyLoad') }}</h2>
            ...
            <div class="p-6">
              ...
              >
                <div
                  ...
                  class="rounded-lg bg-accent-50 p-3 dark:bg-dark-700/50"
                >
                  <div class="flex min-w-0 flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
                    <div class="min-w-0">
                      <div class="flex min-w-0 items-center gap-2">
                        <span class="font-mono text-sm font-semibold text-accent-900 dark:text-white">#{{ item.index + 1 }}</span>
                        <span class="truncate font-mono text-sm text-accent-700 dark:text-accent-200">{{ item.masked || '-' }}</span>
                    ...
                    <div class="grid grid-cols-4 gap-2 text-right text-xs text-accent-500 dark:text-accent-400 sm:min-w-[280px]">
                      <div>
                      ...
                      <div>
                        ...
                        <p class="mt-1 text-sm font-semibold text-accent-900 dark:text-white">{{ formatNumber(item.total) }}</p>
                      </div>
                      <div>
                        <p>{{ t('admin.riskControl.preBlockKeyAvgShort') }}</p>
                        <p class="mt-1 text-sm font-semibold text-accent-900 dark:text-white">{{ formatNumber(item.avg_latency_ms) }} ms</p>
                      </div>
                      <div>
                        <p>{{ t('admin.riskControl.preBlockKeyLastShort') }}</p>
                        <p class="mt-1 text-sm font-semibold text-accent-900 dark:text-white">{{ formatNumber(item.last_latency_ms) }} ms</p>
                      </div>
                    </div>
                  </div>
              ...
              <p v-else class="rounded-lg bg-accent-50 p-4 text-sm text-accent-500 dark:bg-dark-700/50 dark:text-accent-400">
        ...
        <div v-if="showWorkerRuntimeCard" class="card">
          <div class="flex flex-col gap-4 border-b border-accent-100 px-6 py-4 dark:border-dark-700 lg:flex-row lg:items-center lg:justify-between">
            <div>
              <h2 class="text-lg font-semibold text-accent-900 dark:text-white">{{ t('admin.riskControl.workerStatus') }}</h2>
              <p class="mt-1 text-sm text-accent-500 dark:text-accent-400">{{ t('admin.riskControl.workerStatusHint') }}</p>
            </div>
            <div class="flex flex-wrap items-center gap-2 text-sm text-accent-500 dark:text-accent-400">
          ...
          <div class="grid grid-cols-1 gap-6 p-6 xl:grid-cols-[minmax(0,360px)_1fr]">
            <div class="space-y-4">
              <div class="rounded-lg border border-accent-100 p-4 dark:border-dark-700">
                <div class="flex items-center justify-between gap-3">
                  <div>
                    <p class="text-sm font-medium text-accent-900 dark:text-white">{{ t('admin.riskControl.queueUsage') }}</p>
                    <p class="mt-1 text-xs text-accent-500 dark:text-accent-400">
                      {{ formatNumber(status?.queue_length ?? 0) }} / {{ formatNumber(status?.queue_size ?? configForm.queue_size) }}
                    </p>
                  </div>
                  <span class="text-sm font-semibold text-accent-900 dark:text-white">{{ queueUsagePercent }}</span>
                </div>
                <div class="mt-4 h-2 overflow-hidden rounded-full bg-accent-100 dark:bg-dark-700">
                  <div class="h-full rounded-full bg-primary-500 transition-all duration-300" :style="queueUsageStyle"></div>
                </div>
              </div>

              <div class="grid grid-cols-2 gap-3">
                <div class="rounded-lg bg-accent-50 p-4 dark:bg-dark-700/50">
                  <p class="text-xs text-accent-500 dark:text-accent-400">{{ t('admin.riskControl.activeWorkers') }}</p>
                  <p class="mt-2 text-2xl font-semibold text-accent-900 dark:text-white">{{ status?.active_workers ?? 0 }}</p>
                </div>
                <div class="rounded-lg bg-emerald-50 p-4 dark:bg-emerald-900/10">
                  <p class="text-xs text-accent-500 dark:text-accent-400">{{ t('admin.riskControl.idleWorkers') }}</p>
                  <p class="mt-2 text-2xl font-semibold text-emerald-700 dark:text-emerald-300">{{ status?.idle_workers ?? configForm.worker_count }}</p>
                </div>
                <div class="rounded-lg bg-accent-50 p-4 dark:bg-dark-700/50">
                  <p class="text-xs text-accent-500 dark:text-accent-400">{{ t('admin.riskControl.processed') }}</p>
                  <p class="mt-2 text-2xl font-semibold text-accent-900 dark:text-white">{{ formatNumber(status?.processed ?? 0) }}</p>
                </div>
                <div class="rounded-lg bg-accent-50 p-4 dark:bg-dark-700/50">
                  <p class="text-xs text-accent-500 dark:text-accent-400">{{ t('admin.riskControl.droppedErrors') }}</p>
                  <p class="mt-2 text-2xl font-semibold text-accent-900 dark:text-white">{{ formatNumber((status?.dropped ?? 0) + (status?.errors ?? 0)) }}</p>
                </div>
              </div>
            </div>
            ...
            <div>
              <div class="mb-3 flex items-center justify-between gap-3">
                <div>
                  <p class="text-sm font-medium text-accent-900 dark:text-white">{{ t('admin.riskControl.workerPool') }}</p>
                  <p class="mt-1 text-xs text-accent-500 dark:text-accent-400">
                ...
                <span class="inline-flex items-center rounded-full bg-accent-100 px-2.5 py-1 text-xs font-medium text-accent-600 dark:bg-dark-700 dark:text-accent-300">
                  {{ modeLabel(status?.mode ?? configForm.mode) }}
                </span>
              </div>
        ...
        <div class="card">
          <div class="flex flex-col gap-4 border-b border-accent-100 px-6 py-4 dark:border-dark-700">
            <div class="flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
              <div>
                <h2 class="text-lg font-semibold text-accent-900 dark:text-white">{{ t('admin.riskControl.records') }}</h2>
                <p class="mt-1 text-sm text-accent-500 dark:text-accent-400">{{ t('admin.riskControl.recordsHint') }}</p>
              </div>
            ...
            <div class="flex flex-col gap-2 rounded-lg border border-accent-100 bg-accent-50 px-3 py-2 dark:border-dark-700 dark:bg-dark-900/30 sm:flex-row sm:items-center sm:justify-between">
              <div class="flex min-w-0 items-center gap-2 text-sm text-accent-700 dark:text-accent-200">
                <Icon name="filter" size="sm" class="flex-shrink-0 text-accent-400" />
                <span class="font-medium">{{ t('admin.riskControl.modelFilter') }}</span>
                <span class="truncate text-accent-500 dark:text-accent-400">{{ modelFilterSummary }}</span>
              </div>
              <div v-if="modelFilterPreviewModels.length > 0" class="flex flex-wrap gap-1.5">
                <span
                  v-for="model in modelFilterPreviewModels"
                  :key="model"
                  class="inline-flex max-w-[180px] items-center truncate rounded-md bg-white px-2 py-1 font-mono text-xs text-accent-600 shadow-sm dark:bg-dark-800 dark:text-accent-300"
                >
                  {{ model }}
                </span>
                <span v-if="hiddenModelFilterModelCount > 0" class="inline-flex rounded-md bg-white px-2 py-1 text-xs text-accent-500 shadow-sm dark:bg-dark-800 dark:text-accent-400">
          ...
          <div class="overflow-x-auto">
            <table class="min-w-full divide-y divide-accent-200 dark:divide-dark-700">
              <thead class="bg-accent-50 dark:bg-dark-800">
                <tr>
                  <th class="px-5 py-3 text-left text-xs font-medium uppercase tracking-wider text-accent-500 dark:text-accent-400">{{ t('admin.riskControl.table.time') }}</th>
                  <th class="px-5 py-3 text-left text-xs font-medium uppercase tracking-wider text-accent-500 dark:text-accent-400">{{ t('admin.riskControl.table.group') }}</th>
                  <th class="px-5 py-3 text-left text-xs font-medium uppercase tracking-wider text-accent-500 dark:text-accent-400">{{ t('admin.riskControl.table.user') }}</th>
                  <th class="px-5 py-3 text-left text-xs font-medium uppercase tracking-wider text-accent-500 dark:text-accent-400">{{ t('admin.riskControl.table.apiKey') }}</th>
                  <th class="px-5 py-3 text-left text-xs font-medium uppercase tracking-wider text-accent-500 dark:text-accent-400">{{ t('admin.riskControl.table.endpoint') }}</th>
                  <th class="px-5 py-3 text-left text-xs font-medium uppercase tracking-wider text-accent-500 dark:text-accent-400">{{ t('admin.riskControl.table.result') }}</th>
                  <th class="px-5 py-3 text-left text-xs font-medium uppercase tracking-wider text-accent-500 dark:text-accent-400">{{ t('admin.riskControl.table.highest') }}</th>
                  <th class="px-5 py-3 text-left text-xs font-medium uppercase tracking-wider text-accent-500 dark:text-accent-400">{{ t('admin.riskControl.table.actionMeta') }}</th>
                  <th class="px-5 py-3 text-left text-xs font-medium uppercase tracking-wider text-accent-500 dark:text-accent-400">{{ t('admin.riskControl.table.latency') }}</th>
                  <th class="px-5 py-3 text-left text-xs font-medium uppercase tracking-wider text-accent-500 dark:text-accent-400">{{ t('admin.riskControl.table.input') }}</th>
                </tr>
              </thead>
              <tbody class="divide-y divide-accent-100 bg-white dark:divide-dark-800 dark:bg-dark-800">
                <tr v-if="logsLoading">
                  <td colspan="10" class="px-5 py-12 text-center text-sm text-accent-500 dark:text-accent-400">{{ t('common.loading') }}</td>
                </tr>
                <tr v-else-if="logs.length === 0">
                  <td colspan="10" class="px-5 py-12 text-center text-sm text-accent-500 dark:text-accent-400">{{ t('admin.riskControl.emptyLogs') }}</td>
                </tr>
                <template v-else>
                  <tr v-for="row in logs" :key="row.id" class="hover:bg-accent-50 dark:hover:bg-dark-700/60">
                    <td class="whitespace-nowrap px-5 py-4 text-sm text-accent-700 dark:text-accent-300">{{ formatDateTime(row.created_at) }}</td>
                    <td class="whitespace-nowrap px-5 py-4 text-sm text-accent-700 dark:text-accent-300">{{ row.group_name || '-' }}</td>
                    <td class="whitespace-nowrap px-5 py-4 text-sm text-accent-700 dark:text-accent-300">
                      <div>{{ row.user_email || '-' }}</div>
                      <div v-if="row.user_id" class="text-xs text-accent-400">UID {{ row.user_id }}</div>
                    </td>
                    <td class="whitespace-nowrap px-5 py-4 text-sm text-accent-700 dark:text-accent-300">{{ row.api_key_name || '-' }}</td>
                    <td class="whitespace-nowrap px-5 py-4 text-sm text-accent-700 dark:text-accent-300">
                      <div>{{ row.endpoint || '-' }}</div>
                      <div class="text-xs text-accent-400">{{ row.provider || '-' }} / {{ row.model || '-' }}</div>
                    </td>
                    ...
                    <td class="whitespace-nowrap px-5 py-4 text-sm text-accent-700 dark:text-accent-300">
                      ...
                      <div class="text-xs text-accent-400">{{ percent(row.highest_score) }}</div>
                    </td>
                    <td class="whitespace-nowrap px-5 py-4 text-sm text-accent-700 dark:text-accent-300">
                    ...
                    <td class="whitespace-nowrap px-5 py-4 text-sm text-accent-700 dark:text-accent-300">
                      ...
                      <div v-if="row.queue_delay_ms !== null && row.queue_delay_ms !== undefined" class="text-xs text-accent-400">
                        {{ t('admin.riskControl.queueDelay', { ms: row.queue_delay_ms }) }}
                      </div>
                    </td>
                    <td class="w-[320px] max-w-sm px-5 py-4 text-sm text-accent-700 dark:text-accent-300">
                      <button
                        type="button"
                        class="group flex w-full min-w-0 items-center gap-2 rounded-lg px-2 py-1.5 text-left transition-colors hover:bg-accent-100 dark:hover:bg-dark-700"
                        :title="inputSummaryText(row)"
                        @click="openInputDetail(row)"
                      >
                        <span class="min-w-0 flex-1 truncate">{{ inputSummaryText(row) }}</span>
                        <Icon name="eye" size="xs" class="flex-shrink-0 text-accent-300 transition-colors group-hover:text-primary-500 dark:text-accent-500" />
                      </button>
                    </td>
                  </tr>
                </template>
      ...
      <BaseDialog :show="settingsOpen" :title="t('admin.riskControl.settingsTitle')" width="extra-wide" @close="settingsOpen = false">
        <div class="space-y-6">
          <div class="flex gap-2 overflow-x-auto border-b border-accent-100 pb-3 dark:border-dark-700">
            <button
              v-for="tab in settingsTabs"
              :key="tab.id"
              type="button"
              class="inline-flex whitespace-nowrap rounded-lg px-3 py-2 text-sm font-medium transition-colors"
              :class="activeSettingsTab === tab.id ? 'bg-primary-50 text-primary-700 dark:bg-primary-900/30 dark:text-primary-300' : 'text-accent-500 hover:bg-accent-50 hover:text-accent-900 dark:text-accent-400 dark:hover:bg-dark-700 dark:hover:text-white'"
          ...
          <div v-if="activeSettingsTab === 'basic'" class="space-y-5">
            <div class="grid grid-cols-1 gap-5 lg:grid-cols-2">
              <div class="flex items-center justify-between rounded-lg border border-accent-100 p-4 dark:border-dark-700">
                <div>
                  <p class="text-sm font-medium text-accent-900 dark:text-white">{{ t('admin.riskControl.enabled') }}</p>
                  <p class="mt-1 text-xs text-accent-500 dark:text-accent-400">{{ t('admin.riskControl.enabledHint') }}</p>
                </div>
              ...
              <div>
                ...
                <p class="mt-2 text-xs leading-5 text-accent-500 dark:text-accent-400">{{ modeDescription(configForm.mode) }}</p>
              </div>
              <div>
            ...
            <div class="overflow-hidden rounded-xl border border-accent-100 bg-white shadow-sm dark:border-dark-700 dark:bg-dark-800">
              <div class="flex flex-col gap-4 border-b border-accent-100 bg-accent-50 px-4 py-4 dark:border-dark-700 dark:bg-dark-800/60 lg:flex-row lg:items-center lg:justify-between">
                <div class="flex items-start gap-3">
                  ...
                  <div>
                    <label class="text-sm font-semibold text-accent-900 dark:text-white">{{ t('admin.riskControl.apiKeys') }}</label>
                    <p class="mt-1 max-w-3xl text-xs leading-5 text-accent-500 dark:text-accent-400">
              ...
              <div class="grid grid-cols-1 gap-4 p-4 xl:grid-cols-[minmax(0,1fr)_minmax(360px,440px)]">
                <div class="space-y-3">
                  <div class="flex flex-col gap-2 rounded-lg border border-accent-100 bg-accent-50 p-2 dark:border-dark-700 dark:bg-dark-900/30 sm:flex-row sm:items-center sm:justify-between">
                    <div class="text-xs leading-5 text-accent-500 dark:text-accent-400">
                      <span class="font-medium text-accent-700 dark:text-accent-200">{{ t('admin.riskControl.apiKeysWriteMode') }}</span>
                    ...
                    <div class="inline-flex rounded-lg bg-white p-1 shadow-sm dark:bg-dark-800">
                      <button
                        ...
                        :class="configForm.api_keys_mode === 'append' ? 'bg-primary-500 text-white shadow-sm' : 'text-accent-600 hover:bg-accent-100 dark:text-accent-300 dark:hover:bg-dark-700'"
                      ...
                      <button
                        type="button"
                        class="rounded-md px-3 py-1.5 text-xs font-medium transition-colors"
                        :class="configForm.api_keys_mode === 'replace' ? 'bg-amber-500 text-white shadow-sm' : 'text-accent-600 hover:bg-accent-100 dark:text-accent-300 dark:hover:bg-dark-700'"
                        :disabled="configForm.clear_api_key"
                  ...
                  <textarea
                    ...
                    :disabled="configForm.clear_api_key"
                  ></textarea>
                  <div class="flex flex-wrap items-center gap-2 text-xs text-accent-500 dark:text-accent-400">
                    <span class="inline-flex rounded-md bg-accent-100 px-2 py-1 dark:bg-dark-700">
                      {{ t('admin.riskControl.inputApiKeyCount', { count: inputApiKeyCount }) }}
                    </span>
                    <span v-if="configForm.api_key_configured" class="inline-flex rounded-md bg-accent-100 px-2 py-1 dark:bg-dark-700">
                  ...
                  <div class="rounded-lg border border-accent-100 bg-accent-50 p-3 dark:border-dark-700 dark:bg-dark-900/30" @paste="handleModerationImagePaste">
                    <div class="mb-3 flex items-center justify-between gap-3">
                      <div>
                        <p class="text-sm font-semibold text-accent-900 dark:text-white">{{ t('admin.riskControl.auditTestInput') }}</p>
                        <p class="mt-1 text-xs text-accent-500 dark:text-accent-400">{{ t('admin.riskControl.auditTestInputHint') }}</p>
                      </div>
                      <button
                        v-if="moderationTestPrompt || moderationTestImages.length > 0 || moderationTestResult"
                        type="button"
                        class="inline-flex items-center gap-1 rounded-md px-2 py-1 text-xs font-medium text-accent-500 hover:bg-white hover:text-accent-900 dark:text-accent-400 dark:hover:bg-dark-800 dark:hover:text-white"
                    ...
                    ></textarea>
                    <div
                      class="mt-3 rounded-lg border border-dashed border-accent-200 bg-white p-3 dark:border-dark-700 dark:bg-dark-800"
                      @dragover.prevent
                      @drop.prevent="handleModerationImageDrop"
                    >
                      <div class="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                        <div class="flex items-start gap-2">
                          <Icon name="upload" size="md" class="mt-0.5 text-accent-400" />
                          <div>
                            <p class="text-sm font-medium text-accent-800 dark:text-accent-100">{{ t('admin.riskControl.auditTestImages') }}</p>
                            <p class="mt-1 text-xs text-accent-500 dark:text-accent-400">{{ t('admin.riskControl.auditTestImagesHint') }}</p>
                          </div>
                        </div>
                      ...
                      <div v-if="moderationTestImages.length > 0" class="mt-3 grid grid-cols-2 gap-2 sm:grid-cols-4">
                        <div
                          ...
                          class="group relative aspect-square overflow-hidden rounded-lg border border-accent-100 bg-accent-100 dark:border-dark-700 dark:bg-dark-700"
                        >
                ...
                <div class="rounded-lg border border-accent-100 bg-accent-50 p-3 dark:border-dark-700 dark:bg-dark-900/30">
                  <div class="mb-3 flex items-start justify-between gap-3">
                    <div class="min-w-0">
                      <p class="text-sm font-semibold text-accent-900 dark:text-white">{{ t('admin.riskControl.apiKeyHealth') }}</p>
                      <p class="mt-1 text-xs text-accent-500 dark:text-accent-400">{{ t('admin.riskControl.apiKeyFreezeRule') }}</p>
                    </div>
                    <span class="inline-flex shrink-0 items-center whitespace-nowrap rounded-full bg-white px-2 py-0.5 text-[11px] font-medium leading-5 text-accent-600 shadow-sm dark:bg-dark-800 dark:text-accent-300">
                      {{ t('admin.riskControl.apiKeyRows', { count: apiKeyRows.length }) }}
                    </span>
                  </div>

                  <div v-if="apiKeyRows.length === 0" class="flex min-h-32 flex-col items-center justify-center rounded-lg border border-dashed border-accent-200 bg-white px-4 py-6 text-center dark:border-dark-700 dark:bg-dark-800">
                    <Icon name="infoCircle" size="lg" class="text-accent-300 dark:text-dark-500" />
                    <p class="mt-2 text-sm font-medium text-accent-700 dark:text-accent-200">{{ t('admin.riskControl.apiKeyHealthEmpty') }}</p>
                    <p class="mt-1 text-xs text-accent-500 dark:text-accent-400">{{ t('admin.riskControl.apiKeyHealthEmptyHint') }}</p>
                  </div>
                  <div v-else class="space-y-2">
                    <div class="space-y-2" :class="apiKeyRowsExpanded ? 'max-h-72 overflow-y-auto pr-1' : ''">
                      <div
                        ...
                        :class="isStoredApiKeyPendingDelete(row) ? 'border-amber-200 opacity-70 dark:border-amber-800/60' : 'border-accent-100 dark:border-dark-700'"
                      >
                        <div class="flex items-start justify-between gap-2">
                          <div class="min-w-0">
                            <div class="flex min-w-0 flex-wrap items-center gap-2">
                              <span class="truncate font-mono text-sm font-semibold text-accent-900 dark:text-white">{{ row.masked || '-' }}</span>
                              <span
                                class="inline-flex rounded-md px-1.5 py-0.5 text-[11px] font-medium"
                                :class="row.configured ? 'bg-primary-50 text-primary-700 dark:bg-primary-900/30 dark:text-primary-300' : 'bg-accent-50 text-accent-700 dark:bg-accent-900/30 dark:text-accent-300'"
                              >
                            ...
                            <p class="mt-1 text-xs leading-5 text-accent-500 dark:text-accent-400">{{ apiKeyStatusMeta(row) }}</p>
                          </div>
                          <div class="flex flex-shrink-0 items-center gap-1.5">
                            ...
                            <button
                              ...
                              type="button"
                              class="inline-flex h-7 w-7 items-center justify-center rounded-md text-accent-400 transition-colors hover:bg-accent-100 hover:text-accent-700 dark:hover:bg-dark-700 dark:hover:text-accent-200"
                    ...
                    <div v-if="canToggleApiKeyRows" class="flex items-center justify-between gap-3 rounded-lg border border-dashed border-accent-200 bg-white px-3 py-2 text-xs text-accent-500 dark:border-dark-700 dark:bg-dark-800 dark:text-accent-400">
                  ...
                  <div v-if="moderationTestResult" class="mt-4 rounded-lg border border-accent-100 bg-white p-3 dark:border-dark-700 dark:bg-dark-800">
                    <div class="flex items-start justify-between gap-3">
                      <div>
                        <p class="text-sm font-semibold text-accent-900 dark:text-white">{{ t('admin.riskControl.auditTestResult') }}</p>
                        <p class="mt-1 text-xs text-accent-500 dark:text-accent-400">
                    ...
                    <div class="mt-3">
                      <div class="mb-2 flex items-center justify-between text-xs text-accent-500 dark:text-accent-400">
                        <span>{{ t('admin.riskControl.auditTestComposite') }}</span>
                        <span class="font-semibold text-accent-900 dark:text-white">{{ percent(moderationTestResult.composite_score) }}</span>
                      </div>
                      <div class="h-2 overflow-hidden rounded-full bg-accent-100 dark:bg-dark-700">
                    ...
                    <div class="mt-3 max-h-52 space-y-2 overflow-y-auto pr-1">
                      <div v-for="score in moderationScoreRows" :key="score.category">
                        <div class="mb-1 flex items-center justify-between gap-3 text-xs">
                          <span class="truncate text-accent-600 dark:text-accent-300">{{ score.category }}</span>
                          <span class="font-mono text-accent-500 dark:text-accent-400">{{ percent(score.score) }} / {{ percent(score.threshold) }}</span>
                        </div>
                        <div class="h-1.5 overflow-hidden rounded-full bg-accent-100 dark:bg-dark-700">
          ...
          <div v-else-if="activeSettingsTab === 'scope'" class="space-y-5">
            <div class="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
              <div>
                <h3 class="text-base font-semibold text-accent-900 dark:text-white">{{ t('admin.riskControl.groupScope') }}</h3>
                <p class="mt-1 text-sm text-accent-500 dark:text-accent-400">{{ t('admin.riskControl.groupScopeHint') }}</p>
              </div>
              <div class="inline-flex rounded-lg bg-accent-100 p-1 dark:bg-dark-700">
                <button
                  type="button"
                  class="rounded-md px-3 py-1.5 text-sm font-medium transition-colors"
                  :class="configForm.all_groups ? 'bg-white text-accent-900 shadow-sm dark:bg-dark-800 dark:text-white' : 'text-accent-500 dark:text-accent-400'"
            ...
            <div v-if="!configForm.all_groups" class="space-y-4">
              <div class="relative">
                <Icon name="search" size="sm" class="pointer-events-none absolute left-3 top-1/2 -translate-y-1/2 text-accent-400" />
              ...
              <div class="grid max-h-[420px] grid-cols-1 gap-3 overflow-y-auto pr-1 md:grid-cols-2 xl:grid-cols-3">
                <button
                  ...
                  :class="isGroupSelected(group.id) ? 'border-primary-300 bg-primary-50 dark:border-primary-700 dark:bg-primary-900/20' : 'border-accent-100 hover:bg-accent-50 dark:border-dark-700 dark:hover:bg-dark-700/60'"
                  @click="toggleGroup(group.id)"
                >
                  <span class="min-w-0">
                    <span class="block truncate text-sm font-semibold text-accent-900 dark:text-white">{{ group.name }}</span>
                    <span class="mt-1 inline-flex rounded-md bg-accent-100 px-2 py-0.5 text-xs text-accent-500 dark:bg-dark-700 dark:text-accent-400">{{ group.platform }}</span>
                  </span>
                  <span
                    class="flex h-5 w-5 flex-shrink-0 items-center justify-center rounded-full border"
                    :class="isGroupSelected(group.id) ? 'border-primary-500 bg-primary-500 text-white' : 'border-accent-300 text-transparent dark:border-dark-500'"
                  >
                    <Icon name="check" size="xs" :stroke-width="2" />
                  </span>
                </button>
                <p v-if="filteredGroups.length === 0" class="text-sm text-accent-500 dark:text-accent-400">{{ t('admin.riskControl.noGroups') }}</p>
              </div>
            </div>

            <div class="space-y-4 rounded-lg border border-accent-100 p-4 dark:border-dark-700">
              <div class="flex flex-col gap-2 lg:flex-row lg:items-center lg:justify-between">
                <div>
                  <h3 class="text-base font-semibold text-accent-900 dark:text-white">{{ t('admin.riskControl.modelFilter') }}</h3>
                  <p class="mt-1 text-sm text-accent-500 dark:text-accent-400">{{ t('admin.riskControl.modelFilterHint') }}</p>
                </div>
                <span class="inline-flex w-fit rounded-md bg-accent-100 px-2.5 py-1 text-xs font-medium text-accent-600 dark:bg-dark-700 dark:text-accent-300">
                  {{ modelFilterSummary }}
                </span>
              </div>
              ...
              <div class="grid grid-cols-1 gap-2 md:grid-cols-3">
                <button
                  ...
                  :class="configForm.model_filter_type === option.value
                    ...
                    : 'border-accent-100 hover:bg-accent-50 dark:border-dark-700 dark:hover:bg-dark-700/60'"
                  ...
                  <div class="flex items-center justify-between gap-2">
                    ...
                    <span
                      ...
                      :class="configForm.model_filter_type === option.value
                        ...
                        : 'border-accent-300 text-transparent dark:border-dark-500'"
                    >
                      <Icon name="check" size="xs" :stroke-width="2" />
                    </span>
                  </div>
                  <p class="mt-1 text-xs leading-5 text-accent-500 dark:text-accent-400">{{ option.description }}</p>
                </button>
              </div>
          ...
          <div v-else-if="activeSettingsTab === 'runtime'" class="grid grid-cols-1 gap-5 lg:grid-cols-2">
            ...
            <div class="flex items-center justify-between rounded-lg border border-accent-100 p-4 dark:border-dark-700 lg:col-span-2">
              <div>
                <p class="text-sm font-medium text-accent-900 dark:text-white">{{ t('admin.riskControl.recordNonHits') }}</p>
                <p class="mt-1 text-xs text-accent-500 dark:text-accent-400">{{ t('admin.riskControl.recordNonHitsHint') }}</p>
              </div>
              <Toggle v-model="configForm.record_non_hits" />
            </div>
            <div class="space-y-4 rounded-lg border border-accent-100 p-4 dark:border-dark-700 lg:col-span-2">
              <div class="flex items-center justify-between gap-4">
                <div>
                  <p class="text-sm font-medium text-accent-900 dark:text-white">{{ t('admin.riskControl.preHashCheck') }}</p>
                  <p class="mt-1 text-xs text-accent-500 dark:text-accent-400">{{ t('admin.riskControl.preHashCheckHint') }}</p>
                </div>
                <Toggle v-model="configForm.pre_hash_check_enabled" />
              </div>
              <div class="rounded-lg bg-accent-50 p-3 dark:bg-dark-900/30">
                <div class="flex flex-col gap-3 xl:flex-row xl:items-center xl:justify-between">
                  <div>
                    ...
                    <p class="mt-1 text-xs text-accent-500 dark:text-accent-400">{{ t('admin.riskControl.flaggedHashHint') }}</p>
                  </div>
                  <button
                    type="button"
          ...
          <div v-else-if="activeSettingsTab === 'response'" class="space-y-5">
            <div class="grid grid-cols-1 gap-5 lg:grid-cols-2">
              ...
              <div class="flex items-center justify-between rounded-lg border border-accent-100 p-4 dark:border-dark-700">
                <div>
                  <p class="text-sm font-medium text-accent-900 dark:text-white">{{ t('admin.riskControl.emailOnHit') }}</p>
                  <p class="mt-1 text-xs text-accent-500 dark:text-accent-400">{{ t('admin.riskControl.emailOnHitHint') }}</p>
                </div>
                <Toggle v-model="configForm.email_on_hit" />
              </div>
              <div class="flex items-center justify-between rounded-lg border border-accent-100 p-4 dark:border-dark-700">
                <div>
                  <p class="text-sm font-medium text-accent-900 dark:text-white">{{ t('admin.riskControl.autoBan') }}</p>
                  <p class="mt-1 text-xs text-accent-500 dark:text-accent-400">{{ t('admin.riskControl.autoBanHint') }}</p>
                </div>
                <Toggle v-model="configForm.auto_ban_enabled" />
              </div>
              <div class="flex items-center justify-between rounded-lg border border-accent-100 p-4 dark:border-dark-700 lg:col-span-2">
                <div>
                  <p class="text-sm font-medium text-accent-900 dark:text-white">{{ t('admin.riskControl.cyberPolicyExcludeBan') }}</p>
                  <p class="mt-1 text-xs text-accent-500 dark:text-accent-400">{{ t('admin.riskControl.cyberPolicyExcludeBanHint') }}</p>
                </div>
          ...
          <div v-else-if="activeSettingsTab === 'riskThresholds'" class="space-y-5">
            <div class="flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
              <div>
                <h3 class="text-base font-semibold text-accent-900 dark:text-white">{{ t('admin.riskControl.riskThresholds') }}</h3>
                <p class="mt-1 text-sm text-accent-500 dark:text-accent-400">{{ t('admin.riskControl.riskThresholdsHint') }}</p>
              </div>
              <button
                type="button"
                class="btn btn-secondary inline-flex items-center justify-center gap-2"
            ...
            <div class="grid grid-cols-1 gap-3 md:grid-cols-2 xl:grid-cols-3">
              <div
                ...
                :key="row.category"
                class="rounded-lg border border-accent-100 bg-accent-50 p-4 dark:border-dark-700 dark:bg-dark-900/30"
              >
                <div class="flex items-start justify-between gap-3">
                  <div class="min-w-0">
                    <label class="block truncate text-sm font-semibold text-accent-900 dark:text-white" :for="`risk-threshold-${row.category}`">
                      {{ row.category }}
                    </label>
                    <p class="mt-1 text-xs text-accent-500 dark:text-accent-400">
                  ...
                  <span class="inline-flex shrink-0 rounded-md bg-white px-2 py-1 font-mono text-xs font-medium text-accent-600 shadow-sm dark:bg-dark-800 dark:text-accent-300">
          ...
          <div v-else-if="activeSettingsTab === 'keywords'" class="space-y-5">
            ...
            >
              ...
              <div class="text-sm leading-6">
                ...
                <p class="mt-1 text-xs text-accent-500 dark:text-accent-400">{{ keywordNotice.description }}</p>
              </div>
            </div>
            ...
            <div>
              <div class="mb-2 flex items-center justify-between">
                ...
                <span class="inline-flex rounded-md bg-accent-100 px-2 py-1 text-xs text-accent-500 dark:bg-dark-700 dark:text-accent-300">
          ...
          <div v-else class="grid grid-cols-1 gap-5 lg:grid-cols-2">
            ...
            <div class="rounded-lg border border-accent-100 p-4 text-sm text-accent-500 dark:border-dark-700 dark:text-accent-400 lg:col-span-2">
              <div class="flex flex-wrap items-center gap-3">
                <Icon name="database" size="md" class="text-accent-400" />
      ...
      >
        <div v-if="inputDetailRow" class="space-y-5">
          <div class="grid grid-cols-1 gap-3 sm:grid-cols-2 lg:grid-cols-4">
            <div class="rounded-lg border border-accent-100 bg-accent-50 p-4 dark:border-dark-700 dark:bg-dark-800/70">
              <p class="text-xs font-medium text-accent-500 dark:text-accent-400">{{ t('admin.riskControl.table.time') }}</p>
              <p class="mt-1 truncate text-sm font-semibold text-accent-900 dark:text-white">{{ formatDateTime(inputDetailRow.created_at) }}</p>
            </div>
            <div class="rounded-lg border border-accent-100 bg-accent-50 p-4 dark:border-dark-700 dark:bg-dark-800/70">
              <p class="text-xs font-medium text-accent-500 dark:text-accent-400">{{ t('admin.riskControl.table.user') }}</p>
              <p class="mt-1 truncate text-sm font-semibold text-accent-900 dark:text-white">{{ inputDetailRow.user_email || '-' }}</p>
            </div>
            <div class="rounded-lg border border-accent-100 bg-accent-50 p-4 dark:border-dark-700 dark:bg-dark-800/70">
              <p class="text-xs font-medium text-accent-500 dark:text-accent-400">{{ t('admin.riskControl.table.result') }}</p>
            ...
            <div class="rounded-lg border border-accent-100 bg-accent-50 p-4 dark:border-dark-700 dark:bg-dark-800/70">
              <p class="text-xs font-medium text-accent-500 dark:text-accent-400">{{ t('admin.riskControl.table.highest') }}</p>
              <p class="mt-1 truncate text-sm font-semibold text-accent-900 dark:text-white">
                {{ inputDetailRow.highest_category || '-' }} / {{ percent(inputDetailRow.highest_score) }}
              </p>
            </div>
          </div>

          <div class="rounded-xl border border-accent-100 bg-white p-4 shadow-sm dark:border-dark-700 dark:bg-dark-800">
            <div class="flex flex-wrap items-center justify-between gap-3">
              <div>
                <p class="text-sm font-semibold text-accent-900 dark:text-white">{{ t('admin.riskControl.inputDetailContent') }}</p>
                <p class="mt-1 text-xs text-accent-500 dark:text-accent-400">
                  {{ inputDetailRow.endpoint || '-' }} · {{ inputDetailRow.provider || '-' }} / {{ inputDetailRow.model || '-' }}
                </p>
              </div>
              <span v-if="inputDetailRow.group_name" class="inline-flex rounded-md bg-primary-50 px-2.5 py-1 text-xs font-medium text-primary-700 dark:bg-primary-900/20 dark:text-primary-300">
                {{ inputDetailRow.group_name }}
              </span>
            </div>
            <pre class="mt-4 max-h-[420px] overflow-auto whitespace-pre-wrap break-words rounded-lg bg-accent-950 p-4 text-sm leading-6 text-accent-100 shadow-inner dark:bg-black/50">{{ inputDetailText }}</pre>
          </div>
        </div>
...
const overviewItems = computed<OverviewItem[]>(() => [
  {
    ...
    iconClass: configForm.enabled
      ...
      : 'bg-accent-100 text-accent-500 dark:bg-dark-700 dark:text-accent-400',
...
const preBlockMetricItems = computed(() => [
  ...
  },
  {
    ...
    class: 'bg-accent-50 dark:bg-dark-700/50',
    valueClass: 'text-accent-900 dark:text-white',
  },
  {
    key: 'allowed',
...
function resultBadgeClass(row: ContentModerationLog): string {
  ...
  if (row.flagged) return 'bg-accent-100 text-accent-700 dark:bg-accent-900/30 dark:text-accent-300'
...
function workerSlotClass(state: WorkerSlotState): string {
  ...
  return 'border-accent-100 bg-white text-accent-400 dark:border-dark-700 dark:bg-dark-800 dark:text-accent-500'
}
...
function workerDotClass(state: WorkerSlotState): string {
  ...
  return 'bg-accent-300 dark:bg-dark-500'
}
...
function apiKeyStatusBadgeClass(statusValue: ContentModerationAPIKeyStatus['status']): string {
  const classes: Record<ContentModerationAPIKeyStatus['status'], string> = {
    ...
    unknown: 'bg-accent-100 text-accent-600 dark:bg-dark-700 dark:text-accent-300',
  }
