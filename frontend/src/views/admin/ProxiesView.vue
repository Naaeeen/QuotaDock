<template>
  <AppLayout>
      ...
      <template #table>
        ...
        >
          ...
          <template #cell-protocol="{ value }">
            <span
              v-if="value"
              :class="['badge', value.startsWith('socks5') ? 'badge-primary' : 'badge-neutral']"
            >
              {{ value.toUpperCase() }}
            </span>
          ...
          <template #cell-expiry="{ row }">
            <span v-if="!row.expires_at" class="text-sm text-accent-400">{{ t('admin.proxies.neverExpires') }}</span>
            <div v-else class="flex flex-col text-xs">
              <span class="text-accent-700 dark:text-accent-200">{{ formatDateTime(row.expires_at) }}</span>
              <span :class="expiryBadgeClass(row)">{{ expiryLabel(row) }}</span>
            </div>
          </template>

          <template #cell-created_at="{ row }">
            <span class="text-xs text-accent-600 dark:text-accent-300">{{ formatDateTime(row.created_at) }}</span>
          </template>
          ...
          <template #cell-actions="{ row }">
            <div class="flex items-center gap-1">
              ...
              <button
                ...
                class="flex flex-col items-center gap-0.5 rounded-lg p-1.5 text-accent-500 transition-colors hover:bg-primary-50 hover:text-primary-600 disabled:cursor-not-allowed disabled:opacity-50 dark:hover:bg-primary-900/20 dark:hover:text-primary-400"
              >
                <svg
    ...
    >
      <!-- Tab Switch -->
      <div
        class="mb-6 flex items-center justify-between gap-3 border-b border-accent-200 dark:border-dark-600"
      >
...
const handleExportData = async () => {
  ...
  try {
    ...
    const filename = `quotadock-proxy-${timestamp}.json`
