<template>
  <AppLayout>
    <TablePageLayout>
          ...
          <div class="flex items-center gap-3">
            ...
            <div class="min-w-0 flex-1">
              ...
              <p class="text-xs text-accent-500 dark:text-accent-400">
                ...
                <span> · </span>
                <span class="text-primary-600 dark:text-primary-400">{{ t('usage.cacheHit') }} {{ formatTokens(usageStats?.total_cache_read_tokens || 0) }}</span>
                <span> · </span>
              ...
              <p class="text-xs text-accent-400 dark:text-accent-500">
                ...
                <template v-if="cacheStats.totalInput > 0">
                  <span class="text-primary-600 dark:text-primary-400">{{ formatTokens(cacheStats.cacheRead) }}</span>
                  <span class="text-accent-400">/</span>
                  <span class="text-accent-600 dark:text-accent-300">{{ formatTokens(cacheStats.totalInput) }}</span>
        ...
        <div class="card p-4">
          <div class="flex items-center gap-3">
            <div class="rounded-lg bg-accent-100 p-2 dark:bg-accent-900/30">
              <Icon name="clock" size="md" class="text-accent-600 dark:text-accent-400" />
            </div>
            <div>
              <p class="text-xs font-medium text-accent-500 dark:text-accent-400">
                {{ t('usage.avgDuration') }}
      ...
      <template #table>
        <!-- Tab 切换栏 -->
        <div v-if="errorViewEnabled" class="mb-0 flex gap-2 border-b border-accent-200 px-4 pt-3 dark:border-dark-700">
        ...
        >
          ...
          <template #cell-tokens="{ row }">
            ...
            <div v-if="isImageUsage(row)" class="flex items-center gap-1.5">
              ...
              <span class="text-accent-400">({{ formatImageBillingSize(row, t) }})</span>
            </div>
            <!-- Token 请求 -->
            <div v-else class="flex items-center gap-1.5">
              <div class="space-y-1.5 text-sm">
                ...
                <div class="flex items-center gap-2">
                  ...
                  <!-- Output -->
                  <div class="inline-flex items-center gap-1">
                    <Icon name="arrowUp" size="sm" class="text-accent-500" />
                    <span class="font-medium text-accent-900 dark:text-white">{{
                ...
                >
                  ...
                  <div v-if="row.cache_read_tokens > 0" class="inline-flex items-center gap-1">
                    <Icon name="inbox" size="sm" class="text-primary-500" />
                ...
                <div v-if="hasImageOutputTokens(row)" class="flex items-center gap-2">
                  <div class="inline-flex items-center gap-1">
                    ...
                    <span class="font-medium text-accent-600 dark:text-accent-400">{{ row.image_output_tokens.toLocaleString() }}</span>
                  </div>
                </div>
              </div>
              ...
              >
                <div
                  class="flex h-4 w-4 cursor-help items-center justify-center rounded-full bg-accent-100 transition-colors group-hover:bg-primary-100 dark:bg-accent-700 dark:group-hover:bg-primary-900/50"
                >
                  <Icon
                    name="infoCircle"
                    size="xs"
                    class="text-accent-400 group-hover:text-primary-500 dark:text-accent-500 dark:group-hover:text-primary-400"
                  />
                </div>
              </div>
            </div>
        ...
        <div class="space-y-1.5">
          ...
          <div>
            ...
            <div v-if="tokenTooltipData && hasImageOutputTokens(tokenTooltipData)" class="flex items-center justify-between gap-4">
              <span class="text-accent-400">{{ t('usage.imageOutputTokens') }}</span>
        ...
        <div class="space-y-1.5">
          ...
          <div class="mb-2 border-b border-accent-700 pb-1.5">
            ...
            <div v-if="tooltipData && hasImageOutputCost(tooltipData)" class="flex items-center justify-between gap-4">
              <span class="text-accent-400">{{ t('usage.imageOutputCost') }}</span>
            ...
            <template v-if="!tooltipData?.billing_mode || tooltipData.billing_mode === BILLING_MODE_TOKEN">
              <div v-if="tooltipData && tooltipData.input_tokens > 0" class="flex items-center justify-between gap-4">
                ...
                <span class="font-medium text-primary-300">{{ formatTokenPricePerMillion(tooltipData.input_cost, tooltipData.input_tokens) }} {{ t('usage.perMillionTokens') }}</span>
              </div>
              ...
              <div v-if="tooltipData && hasImageOutputTokens(tooltipData)" class="flex items-center justify-between gap-4">
                <span class="text-accent-400">{{ t('usage.imageOutputTokenPrice') }}</span>
            ...
            <template v-else-if="tooltipData && isImageUsage(tooltipData)">
              ...
              <div v-if="formatImageSizeBreakdown(tooltipData)" class="flex items-center justify-between gap-4">
                <span class="text-accent-400">{{ t('usage.imageSizeBreakdown') }}</span>
                <span class="font-medium text-white">{{ formatImageSizeBreakdown(tooltipData) }}</span>
              </div>
              <div class="flex items-center justify-between gap-4">
                <span class="text-accent-400">{{ t('usage.imageUnitPrice') }}</span>
                <span class="font-medium text-primary-300">${{ imageUnitPrice(tooltipData).toFixed(6) }}</span>
              </div>
              <div class="flex items-center justify-between gap-4">
                <span class="text-accent-400">{{ t('usage.imageTotalPrice') }}</span>
                <span class="font-medium text-white">${{ tooltipData.total_cost?.toFixed(6) || '0.000000' }}</span>
              </div>
            </template>
            <div v-else class="flex items-center justify-between gap-4">
              <span class="text-accent-400">{{ t('usage.unitPrice') }}</span>
              <span class="font-medium text-primary-300">${{ tooltipData?.total_cost?.toFixed(6) || '0.000000' }}</span>
            </div>
...
const getRequestTypeBadgeClass = (log: UsageLog): string => {
  ...
  if (requestType === 'ws_v2') return 'bg-accent-100 text-accent-800 dark:bg-accent-900 dark:text-accent-200'
