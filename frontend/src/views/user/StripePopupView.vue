<template>
  <div class="flex min-h-screen items-center justify-center bg-accent-50 p-4 dark:bg-accent-950">
    <div
      class="w-full max-w-md space-y-4 rounded-2xl border border-accent-200 bg-white p-6 shadow-lg dark:border-accent-700 dark:bg-accent-900"
    >
      <!-- Amount + Order ID -->
      <div v-if="amount" class="text-center">
        <p class="text-3xl font-bold" :style="{ color: methodColor }">¥{{ amount }}</p>
        <p v-if="orderId" class="mt-1 text-sm text-accent-500 dark:text-accent-400">
      ...
      <div v-else-if="success" class="space-y-3 py-4 text-center">
        ...
        <p class="text-sm text-accent-500 dark:text-accent-400">{{ t('payment.result.success') }}</p>
        <button
      ...
      <div v-else class="flex items-center justify-center py-8">
        ...
        />
        <span class="ml-3 text-sm text-accent-500 dark:text-accent-400">{{ hint }}</span>
      </div>
    </div>
  </div>
</template>
