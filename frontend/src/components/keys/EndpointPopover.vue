  ...
  <div v-if="allEndpoints.length > 0" class="flex flex-wrap gap-2">
    ...
    >
      ...
      <div class="group/endpoint relative flex items-center gap-1.5">
        <div
          class="pointer-events-none absolute bottom-full left-1/2 z-20 mb-2 w-max max-w-[24rem] -translate-x-1/2 translate-y-1 rounded-xl border border-accent-200 bg-white px-3 py-2.5 text-left opacity-0 shadow-[0_14px_36px_-20px_rgba(82,58,43,0.35)] ring-1 ring-accent-200/80 transition-all duration-150 group-hover/endpoint:translate-y-0 group-hover/endpoint:opacity-100 group-focus-within/endpoint:translate-y-0 group-focus-within/endpoint:opacity-100 dark:border-accent-700 dark:bg-accent-900 dark:ring-accent-700/70"
        >
          <p
            v-if="item.description"
            class="max-w-[24rem] break-words text-xs leading-5 text-accent-600 dark:text-accent-200"
          >
            {{ item.description }}
          </p>
          <p
          ...
          <div class="absolute left-1/2 top-full h-3 w-3 -translate-x-1/2 -translate-y-1/2 rotate-45 border-b border-r border-accent-200 bg-white dark:border-accent-700 dark:bg-accent-900"></div>
        </div>

        <code
          class="cursor-pointer font-mono text-accent-500 decoration-accent-400 decoration-dashed underline-offset-2 hover:text-primary-600 hover:underline focus:text-primary-600 focus:underline focus:outline-none dark:text-accent-400 dark:decoration-accent-500 dark:hover:text-primary-400 dark:focus:text-primary-400"
          role="button"
          tabindex="0"
