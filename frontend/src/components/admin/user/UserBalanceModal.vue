<template>
  <BaseDialog :show="show" :title="operation === 'add' ? t('admin.users.deposit') : t('admin.users.withdraw')" width="narrow" @close="$emit('close')">
    <form v-if="user" id="balance-form" @submit.prevent="handleBalanceSubmit" class="space-y-5">
      ...
      <div v-if="form.amount > 0" class="rounded-xl border border-primary-200 bg-primary-50 p-4 dark:border-primary-800 dark:bg-primary-950"><div class="flex items-center justify-between text-sm"><span class="text-accent-700 dark:text-accent-300">{{ t('admin.users.newBalance') }}:</span><span class="font-bold text-accent-900 dark:text-accent-100">${{ formatBalance(calculateNewBalance()) }}</span></div></div>
    </form>
    <template #footer>
