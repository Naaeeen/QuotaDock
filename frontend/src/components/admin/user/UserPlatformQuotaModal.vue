<template>
  ...
  >
    <div v-if="user" class="space-y-4">
      ...
      <div v-if="loading" class="py-10 text-center text-accent-500">{{ t('common.loading') }}</div>
      <div v-else class="overflow-x-auto">
        <table class="min-w-full text-sm">
          <thead>
            <tr class="border-b border-accent-200 text-accent-700 dark:border-dark-700 dark:text-accent-300">
          ...
          <tbody>
            <tr v-for="row in quotas" :key="row.platform" class="border-b border-accent-100 dark:border-dark-800">
              <td class="px-3 py-2 font-mono text-accent-900 dark:text-white">{{ row.platform }}</td>
              <td class="px-3 py-2">
                <div class="flex items-center gap-1">
                  ...
                  <button
                    type="button"
                    class="text-xs text-accent-400 hover:text-amber-500 disabled:opacity-50"
              ...
              <td class="px-3 py-2 text-xs text-accent-500 dark:text-accent-400">
                {{ formatUsage(row.daily_usage_usd) }} / {{ formatUsage(row.weekly_usage_usd) }} / {{ formatUsage(row.monthly_usage_usd) }}
              </td>
            </tr>
          </tbody>
        </table>
        <p class="mt-3 text-xs text-accent-500">{{ t('admin.users.platformQuota.hint') }}</p>
        <div class="mt-3">
