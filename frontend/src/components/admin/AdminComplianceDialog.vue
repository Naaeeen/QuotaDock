  ...
  >
    <div class="space-y-5">
      ...
      <div class="grid gap-4 md:grid-cols-[minmax(0,1fr)_240px]">
        <section class="min-h-[320px] max-h-[46vh] overflow-y-auto rounded-lg border border-accent-200 bg-white p-5 dark:border-dark-700 dark:bg-dark-900">
          <div class="legal-document-content" v-html="renderedDocument"></div>
        </section>

        <aside class="space-y-3 rounded-lg border border-accent-200 bg-accent-50 p-4 text-sm dark:border-dark-700 dark:bg-dark-900/60">
          <div>
            <p class="text-xs font-medium uppercase tracking-wide text-accent-500 dark:text-dark-400">
              {{ t('adminCompliance.version') }}
            </p>
            <p class="mt-1 break-all font-mono text-accent-900 dark:text-white">
          ...
          <p class="leading-6 text-accent-600 dark:text-dark-300">
      ...
      <div class="space-y-3">
        <label for="admin-compliance-phrase" class="block text-sm font-semibold text-accent-900 dark:text-white">
          {{ t('adminCompliance.inputLabel') }}
        </label>
        <div class="rounded-lg bg-accent-100 px-3 py-2 font-mono text-sm text-accent-900 dark:bg-dark-800 dark:text-dark-100">
          {{ expectedPhrase }}
        </div>
        <Input
      ...
      <p class="text-xs leading-5 text-accent-500 dark:text-dark-400">
...
.legal-document-content :deep(h1) {
  @apply mb-4 text-2xl font-bold text-accent-950 dark:text-white;
}
...
.legal-document-content :deep(p) {
  @apply mb-4 text-sm text-accent-700 dark:text-dark-200;
}

.legal-document-content :deep(ul),
.legal-document-content :deep(ol) {
  @apply mb-4 pl-6 text-sm text-accent-700 dark:text-dark-200;
}
...
.legal-document-content :deep(strong) {
  @apply font-semibold text-accent-950 dark:text-white;
}
</style>
