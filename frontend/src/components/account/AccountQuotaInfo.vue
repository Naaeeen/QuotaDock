...
const tierBadgeClass = computed(() => {
  ...
  if (isCodeAssist.value) {
    const tier = (creds?.tier_id || '').toString().trim().toLowerCase()
    if (tier === 'gcp_enterprise') return 'bg-accent-100 text-accent-600 dark:bg-accent-900/40 dark:text-accent-300'
    if (tier === 'gcp_standard') return 'bg-primary-100 text-primary-600 dark:bg-primary-900/40 dark:text-primary-300'
    // Backward compatibility
    const upper = (creds?.tier_id || '').toString().trim().toUpperCase()
    if (upper.includes('ULTRA') || upper.includes('ENTERPRISE')) return 'bg-accent-100 text-accent-600 dark:bg-accent-900/40 dark:text-accent-300'
    return 'bg-primary-100 text-primary-600 dark:bg-primary-900/40 dark:text-primary-300'
  }

  if (isGoogleOne.value) {
    const tier = (creds?.tier_id || '').toString().trim().toLowerCase()
    if (tier === 'google_ai_ultra') return 'bg-accent-100 text-accent-600 dark:bg-accent-900/40 dark:text-accent-300'
    if (tier === 'google_ai_pro') return 'bg-primary-100 text-primary-600 dark:bg-primary-900/40 dark:text-primary-300'
    if (tier === 'google_one_free') return 'bg-accent-100 text-accent-600 dark:bg-accent-700 dark:text-accent-300'
    // Backward compatibility
    const upper = (creds?.tier_id || '').toString().trim().toUpperCase()
    if (upper === 'GOOGLE_ONE_UNLIMITED') return 'bg-accent-100 text-accent-600 dark:bg-accent-900/40 dark:text-accent-300'
    if (upper === 'AI_PREMIUM') return 'bg-primary-100 text-primary-600 dark:bg-primary-900/40 dark:text-primary-300'
    return 'bg-accent-100 text-accent-600 dark:bg-accent-700 dark:text-accent-300'
  }

  // AI Studio 默认样式：蓝色
  const tier = (creds?.tier_id || '').toString().trim().toLowerCase()
  if (tier === 'aistudio_paid') return 'bg-primary-100 text-primary-600 dark:bg-primary-900/40 dark:text-primary-300'
