<template>
  <div class="group/usage relative text-sm">
    <div class="flex items-center gap-1.5">
      ...
      <span class="font-medium text-accent-900 dark:text-white">${{ today.toFixed(4) }}</span>
      <Icon
        v-if="hasBreakdown"
        name="infoCircle"
        size="xs"
    ...
    <div class="mt-0.5 flex items-center gap-1.5">
      ...
      <span class="font-medium text-accent-900 dark:text-white">${{ total.toFixed(4) }}</span>
    </div>

    <div
      v-if="hasBreakdown"
      class="pointer-events-none absolute left-full top-0 z-50 ml-2 min-w-[220px] whitespace-nowrap rounded-md bg-accent-900 px-3 py-2 text-xs text-white opacity-0 shadow-xl transition-opacity duration-100 group-hover/usage:opacity-100 dark:bg-dark-600"
    >
