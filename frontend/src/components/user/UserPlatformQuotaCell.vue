<template>
  <span v-if="props.quotas === undefined" class="text-xs text-accent-400 dark:text-accent-500">…</span>
  <span v-else-if="configured.length === 0" class="text-xs text-accent-400 dark:text-accent-500">
  ...
  <div v-else class="space-y-0.5 text-xs">
    ...
    >
      <span class="w-20 shrink-0 font-mono text-accent-700 dark:text-accent-300">{{ row.platform }}</span>
      <span class="text-accent-500 dark:text-accent-400">
        {{ t('admin.users.platformQuota.windowDaily') }}
        <span class="text-accent-900 dark:text-white">{{ fmtUsd(row.daily_usage_usd) }}/{{ fmtLimit(row.daily_limit_usd) }}</span>
      </span>
      <span class="text-accent-500 dark:text-accent-400">
        {{ t('admin.users.platformQuota.windowWeekly') }}
        <span class="text-accent-900 dark:text-white">{{ fmtUsd(row.weekly_usage_usd) }}/{{ fmtLimit(row.weekly_limit_usd) }}</span>
      </span>
      <span class="text-accent-500 dark:text-accent-400">
        {{ t('admin.users.platformQuota.windowMonthly') }}
        <span class="text-accent-900 dark:text-white">{{ fmtUsd(row.monthly_usage_usd) }}/{{ fmtLimit(row.monthly_limit_usd) }}</span>
      </span>
    </div>
  </div>
</template>
