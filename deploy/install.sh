...
declare -A MSG_ZH=(
    ...
    # Completion
    ["install_complete"]="QuotaDock 安装完成！"
    ["install_dir"]="安装目录"
    ["next_steps"]="后续步骤"
    ["step1_check_services"]="确保 PostgreSQL 和 Redis 正在运行："
    ["step2_start_service"]="启动 QuotaDock 服务："
    ...
    ["not_installed"]="QuotaDock 尚未安装，请先执行全新安装"
    ["fresh_install_hint"]="用法"

    # Uninstall
    ["uninstall_confirm"]="这将从系统中移除 QuotaDock。"
    ...
    ["uninstall_complete"]="QuotaDock 已卸载"

    # Help
    ["usage"]="用法"
    ["cmd_none"]="(无参数)"
    ["cmd_install"]="安装 QuotaDock"
    ["cmd_upgrade"]="升级到最新版本"
    ["cmd_uninstall"]="卸载 QuotaDock"
    ...
    ["server_config_desc"]="配置 QuotaDock 服务监听地址"
...
declare -A MSG_EN=(
    ...
    # Installation
    ["install_title"]="QuotaDock Installation Script"
    ...
    # Completion
    ["install_complete"]="QuotaDock installation completed!"
    ["install_dir"]="Installation directory"
    ["next_steps"]="NEXT STEPS"
    ["step1_check_services"]="Make sure PostgreSQL and Redis are running:"
    ["step2_start_service"]="Start QuotaDock service:"
    ...
    ["not_installed"]="QuotaDock is not installed. Please run a fresh install first"
    ["fresh_install_hint"]="Usage"

    # Uninstall
    ["uninstall_confirm"]="This will remove QuotaDock from your system."
    ...
    ["uninstall_complete"]="QuotaDock has been uninstalled"

    # Help
    ["usage"]="Usage"
    ["cmd_none"]="(none)"
    ["cmd_install"]="Install QuotaDock"
    ["cmd_upgrade"]="Upgrade to the latest version"
    ["cmd_uninstall"]="Remove QuotaDock"
    ...
    ["server_config_desc"]="Configure QuotaDock server listen address"
...
[Unit]
Description=QuotaDock - AI API Gateway Platform
...
# Upgrade function
upgrade() {
    # Check if QuotaDock is installed
    if [ ! -f "$INSTALL_DIR/sub2api" ]; then
...
# Requires: QuotaDock must already be installed
install_version() {
