...
### 目标
本文档用于对接外部支付系统（如 `sub2apipay`）与 QuotaDock 的 Admin API，覆盖：
- 支付成功后充值
- 用户查询
- 人工余额修正
- 前端购买页参数透传
...
当 QuotaDock 打开 `purchase_subscription_url` 或用户侧自定义页面 iframe URL 时，会统一追加：
- `user_id`
- `token`
- `theme`（`light` / `dark`）
...
### Purpose
This document describes the minimal QuotaDock Admin API surface for external payment integrations (for example, `sub2apipay`), including:
...
When QuotaDock opens `purchase_subscription_url` or a user-facing custom page iframe URL, it appends:
- `user_id`
- `token`
- `theme` (`light` / `dark`)
