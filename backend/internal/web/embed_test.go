...
func TestInjectSiteTitle(t *testing.T) {
	t.Run("replaces_title_with_site_name", func(t *testing.T) {
		html := []byte(`<html><head><title>QuotaDock - AI API Gateway</title></head><body></body></html>`)
		...
		assert.NotContains(t, string(result), "QuotaDock")
	})
	...
	t.Run("returns_unchanged_when_title_has_attributes", func(t *testing.T) {
		...
		html := []byte(`<html><head><title lang="en">QuotaDock</title></head><body></body></html>`)
	...
	t.Run("preserves_rest_of_html", func(t *testing.T) {
		html := []byte(`<html><head><meta charset="UTF-8"><title>QuotaDock</title><script src="app.js"></script></head><body><div id="app"></div></body></html>`)
